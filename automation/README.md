...
## 3b. Human voices (the voice must NOT sound like AI)

Tuned with the techniques from the user's reference tutorials
(*"How I Actually Make AI Voice Sound Real"* + Julian Treasure's
*"How to Speak So That People Want to Listen"*):

- **ElevenLabs settings** (env-tunable, self-host): `TTS_STABILITY` 0.38
  (35-45% = human variation; higher = robotic), `TTS_SIMILARITY` 0.80,
  `TTS_STYLE` 0.42 (natural emotion), `TTS_MODEL` multilingual v2,
  speaker boost on. The fork's `generate_voiceover` reads these; the
  docker-compose backend service exposes them.
- **Per-segment TTS**: reels >=45s synthesize each script segment separately
  (hook/problem/solution/demo/cta) and concatenate — so delivery follows each
  beat's pacing instead of one long monotone take.
- **Delivery cues baked into every script prompt**: `...` pauses before key
  points (silence = power), CAPS emphasis (max 3/segment), varied sentence
  length, pace changes, one rhetorical question, natural Hinglish, no
  monotone, no upspeak. The Gemini gate checks these too.

## 3c. Lifelike talking avatars (not static images)

The bot's production path uses the OpenShorts generate pipeline — a real
**lip-synced talking head** — not a still image:

- **Low-cost mode**: Hailuo 2.3 Fast motion clip(s) + **VEED Lipsync**. This
  fork adds **chunked lipsync** for 60-120s reels: each script segment gets
  its own lipsync pass on one of **two alternating motion clips** (nods,
  leaning forward, micro-expressions), then the clips are concatenated — the
  avatar's body language shifts through the reel like a real person instead
  of one 6s loop. Falls back to single-pass lipsync automatically.
- **Premium mode**: Kling Avatar v2 Standard (full-audio talking head).
- **Consistent faces**: pin each persona's `avatar_url` (OpenShorts gallery)
  so every reel of that influencer uses the identical face.
- Actor prompts are photorealistic (skin texture, natural light, no AI-gloss).

The static-image sample reel (`automation/sample_reel.py`) remains only as a
keyless format preview; production reels are talking avatars.
...
**Posting muscle**: 6 reels/day (08:00-04:00 IST, every 4 hours) - **each reel
60-120 seconds** (long-format: `REEL_DURATION_MIN/MAX/TARGET` envs, default
target 75s). Consistency beats virality: the algorithm rewards daily posting
from day 1.
...
- **long-format (64-78s each)**, one per topic, each with a distinct influencer
(10 different faces/voices, Hindi⇄English flips, mid-roll questions,
micro-stories, comment CTAs, `...` pauses and CAPS emphasis for human delivery).
They are verified by the Gemini gate, generated by OpenShorts (per-segment TTS
+ chunked lipsync = talking avatar), and uploaded to your shared Google Drive
...
| Item | Approx. | Notes |
|---|---|---|
| fal.ai generation | **~$6/day ≈ $180/mo** (`lowcost`) | Long reels (60-120s): 2 Hailuo motion clips + chunked VEED lipsync + b-roll ≈ $1.0-1.3/reel. `premium` (Kling) ≈ $2-3/reel. |
| OpenShorts hosted plan | $12+/mo + minutes | 6×~75s ≈ 7.5 min/day ≈ 225 min/mo — pick a plan covering that |
| ElevenLabs | **Creator plan (~$22/mo)** | 6 reels × ~75s voice ≈ 450 min/mo of generation — the Creator plan is REQUIRED |
...
**Total ≈ $210-230/mo at 6 long reels/day** — that's the honest operating cost
of this cadence. Start at 6/day only when the page earns it; `POSTS_PER_DAY=2`
costs ≈ $70-80/mo. Dry-run is free (Actions → Reels Bot → Run workflow →
dry_run).
